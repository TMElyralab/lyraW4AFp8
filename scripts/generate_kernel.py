...
DISPATCH_TEMPLATE = """
#include "../machete/machete_mm_launcher.cuh"
...
torch::Tensor mm_dispatch(MMArgs args) {
  ...
  auto maybe_group_layout_type = maybe_scalartype(args.maybe_group_layout);
...
using Kernel_{{type_sig}} = MacheteKernelTemplate<
  ...
  //cutlass::gemm::KernelTmaWarpSpecializedCooperativeMachete,
  cutlass::gemm::KernelTmaWarpSpecializedCooperativeMachete,
...
PREPACK_TEMPLATE = """
#include "../machete/machete_prepack_launcher.cuh"
...
def generate():
    ...
    for BLOCK_M in [128, 256]:
        for BLOCK_N in [16, 32, 64, 128]:
            for CLUSTER_ID in [1, 2]:
                default_tile_heuristic_config["K > {}".format(cnt)] = (
                    (BLOCK_M, BLOCK_N),
                    (CLUSTER_ID, 1, 1),
                )
                cnt += 1
    ...
    default_heuristic += [
        (cond, ScheduleConfig(*tile_config, **sch_persistent_params))  # type: ignore
        for cond, tile_config in default_tile_heuristic_config.items()
    ]
    ...
    AWQ_kernel_type_configs = list(
        ...
        for a in (DataType.f16, DataType.bf16)
        for g in (DataType.s32, DataType.void)
    )
    ...
    AWQ_w4a8_kernel_type_configs = list(
        TypeConfig(
            a=DataType.e4m3,
            b=DataType.u4,
            b_group_scale=a,
        ...
        for a in (DataType.f16, DataType.bf16)
        # for a in (DataType.f16,)
        for g in (DataType.s32, DataType.void)
    )

    AWQ_w4a8_kernel_type_configs_with_epi = list(
        TypeConfig(
            a=DataType.e4m3,
            b=DataType.u4,
            b_group_scale=a,
    ...
    QQQ_kernel_types = [
        ...
        *(
        ...
        # NotImplementedError: machete_mm(..) is not implemented for a_type=Float8_e4m3fn, b_type=uint4b8, out_type=BFloat16, with_group_scale_type=BFloat16, with_group_zeropoint_type=None, with_channel_scale_type=None, with_token_scale_type=None; im
        *(
            TypeConfig(
                a=DataType.e4m3,
                b=VLLMDataType.u4b8,
            ...
            for b_group_scale in (DataType.bf16, DataType.void)
            for g in (DataType.s32, DataType.void)
        ),
        *(
            TypeConfig(
    ...
    output_dir = os.path.join(SCRIPT_DIR, "../csrc/generated")
